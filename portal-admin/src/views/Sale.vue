      ...
      <el-table :data="list" v-loading="loading" stripe style="width:100%;">
        ...
        <el-table-column label="金额(元)" width="140">
          <template slot-scope="{ row }">
            <span style="color:#f59e0b;font-weight:600;">¥{{ Number(row.amount).toLocaleString() }}</span>
          </template>
        </el-table-column>
    ...
    <el-dialog title="新增销售记录" :visible.sync="dialogVisible" width="500px">
      <el-form :model="form" :rules="rules" ref="saleForm" label-width="90px">
        ...
        <el-form-item label="买家">
          <el-input v-model="form.buyer" placeholder="买家信息" />
        </el-form-item>
      ...
      <span slot="footer">
        ...
        <el-button type="primary" @click="handleAdd" :loading="submitting">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
...
<script>
import { getSaleList, addSale, deleteSale, getAllProducts } from '@/api'
...
export default {
  ...
  methods: {
    async loadData() {
      this.loading = true
      try {
        const res = await getSaleList({ pageNum: this.pageNum, pageSize: this.pageSize })
    ...
    handleProductChange(productId) {
      const p = this.products.find(i => i.productId === productId)
      if (p) {
        this.form.amount = Number(p.price) * this.form.quantity
      }
    },
    handleAdd() {
      this.$refs.saleForm.validate(async valid => {
        ...
        try {
          await addSale(this.form)
          this.$message.success('新增成功')
          this.dialogVisible = false
          this.form = { productId: null, quantity: 1, amount: 0, buyer: '', remark: '' }
          this.loadData()
        } finally {
    ...
    handleDelete(row) {
      this.$confirm('确定删除此销售记录？', '提示', { type: 'warning' }).then(async () => {
        await deleteSale(row.recordId)
