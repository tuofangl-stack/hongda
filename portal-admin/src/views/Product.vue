<template>
  <div>
    <div class="page-header">
      <h2>产品管理</h2>
      <el-button type="primary" icon="el-icon-plus" @click="$router.push('/product/add')">
        新增产品
      </el-button>
    </div>
    ...
    <div class="page-card">
      ...
      <el-table :data="list" v-loading="loading" stripe style="width:100%;">
        ...
        <el-table-column prop="price" label="价格(元)" width="130">
          <template slot-scope="{ row }">
            <span style="color:#f59e0b;font-weight:600;">¥{{ Number(row.price).toLocaleString() }}</span>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="160" fixed="right">
          <template slot-scope="{ row }">
            <el-button type="text" @click="$router.push('/product/edit/' + row.productId)">编辑</el-button>
