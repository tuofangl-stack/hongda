  ...
  <div>
    ...
    <div class="chart-panel">
      ...
      <div v-for="(item, index) in topProducts" :key="item.productId" class="top-product-row">
        <span class="top-rank" :class="index===0?'rank-1':index===1?'rank-2':index===2?'rank-3':'rank-other'">{{ index + 1 }}</span>
...
import { getDashboardStats, getDashboardTrend, getTopProducts } from '@/api/adminApi'

export default {
  name: 'AdminDashboard',
  data() {
    return { stats: {}, topProducts: [], viewChartInstance: null, saleChartInstance: null }
  },
  ...
  methods: {
    async loadData() {
      try {
        const [statsRes, trendRes, topRes] = await Promise.all([
          getDashboardStats(), getDashboardTrend(7), getTopProducts()
        ])
    ...
    renderCharts(trend) {
      ...
      this.viewChartInstance.setOption({
        ...
        xAxis: { type: 'category', data: (trend.viewTrend||[]).map(i=>i.date), axisLabel:{fontSize:11} },
        yAxis: { type: 'value', minInterval: 1 },
        series: [{
          data: (trend.viewTrend||[]).map(i=>i.count), type: 'line', smooth: true,
      ...
      this.saleChartInstance.setOption({
        tooltip: { trigger: 'axis', formatter: p => p[0].name+'<br/>销售额: ¥'+Number(p[0].value).toLocaleString() },
        grid: { left: 60, right: 20, top: 20, bottom: 30 },
        xAxis: { type: 'category', data: (trend.saleTrend||[]).map(i=>i.date), axisLabel:{fontSize:11} },
        yAxis: { type: 'value' },
        series: [{
          data: (trend.saleTrend||[]).map(i=>Number(i.amount)), type: 'bar', barWidth: 24,
          itemStyle: { borderRadius: [8,8,0,0], color: new echarts.graphic.LinearGradient(0,0,0,1,[{offset:0,color:'#f59e0b'},{offset:1,color:'#f97316'}]) }
        }]
      })
    },
    formatMoney(val) {
